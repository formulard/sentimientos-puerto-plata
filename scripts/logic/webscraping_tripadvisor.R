#' Extract Places from a TripAdvisor Destination Page
#'
#' This function scrapes place names, ratings, and URLs from a TripAdvisor destination page
#' using live rendering (Chromote), which allows JavaScript-loaded content to be accessed.
#' 
#' @export
get_places_from_destination <- function(page_url, sleep = 3) {
  box::use(
  ...
  tibble(
    ...
    type = str_extract(url, "Attraction|Hotel|Restaurant")
  )
}

#' Scrape Reviews from a TripAdvisor Attraction Page
#'
#' Extracts the title, content, and date of visitor reviews from a TripAdvisor attraction page.
#' Uses live rendering to handle JavaScript content.
#'
#' @param place_url Character. URL of the TripAdvisor attraction page.
#' @param sleep Numeric. Seconds to wait after loading the page to ensure reviews are rendered. Default is 5.
get_attraction_reviews_single_page <- function(place_url, sleep = 5) {
...
#' Scrape Reviews from tripadvisor attraction page
#' 
#' @export
get_attraction_reviews <- function(url, sleep = 5, n_pages = 1) {
  safe_get_attraction_reviews <- purrr::possibly(
    get_attraction_reviews_single_page, 
...
get_place_reviews_single_page <- function(place_url, sleep = 5) {
...
#' Scrape Reviews from tripadvisor Hotels or Restaurants page
#' 
#' @export
get_place_reviews <- function(url, sleep = 5, n_pages = 1) {
  safe_get_place_reviews <- purrr::possibly(
    get_place_reviews_single_page, otherwise = data.frame())
